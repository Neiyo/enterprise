...
public class EnterpriseRequisitionServiceImpl implements EnterpriseRequisitionService {
    ...
    @Override
    @Transactional
    public void payRequisition(long requisitionId, long managerId) {
        ...
        if (req.getStatus() == RequisitionStatus.PASSED) {
            enterpriseAllocationService.singleAllocat(req.getParentId(), managerId, req.getEnterpriseId(), req.getFee(), req.getRemark());
            enterpriseRequisitionMapper.updateStatusByPrimaryKey(requisitionId, RequisitionStatus.PAYED);
        }
    }
