$(document).ready(function(){
    ...
    jPages("../web/api/ordermanage/getOrders","itemContainer_All","holder_All",1,6,'','',array,function(pageId){
    ...
    $("#accSearch").click(function(){
        ...
        if(createEnd>=createStart&&payEnd>=payStart){
            ...
            if( type == '待付款' ){
                ...
                jPages("../web/api/ordermanage/getOrders","itemContainer_first","holder_first",1,6,'','',array,function(pageId){
                    if( checked_array[pageId] != undefined ) {
                        $(".accountNumber").next().each(function () {
                            var _this = this;
                            checked_array[pageId].map(function(orderCode){
            ...
            if( type == '待发货' ){
                ...
                jPages("../web/api/ordermanage/getOrders","itemContainer_second","holder_second",1,6,'','',array,function(pageId){
                    if( checked_array[pageId] != undefined ) {
                        $(".accountNumber").next().each(function () {
                            var _this = this;
                            checked_array[pageId].map(function(orderCode){
            ...
            if( type == '待收货' ){
                ...
                jPages("../web/api/ordermanage/getOrders","itemContainer_third","holder_third",1,6,'','',array,function(pageId){
                    if( checked_array[pageId] != undefined ) {
                        $(".accountNumber").next().each(function () {
                            var _this = this;
                            checked_array[pageId].map(function(orderCode){
            ...
            if( type == '待提货' ){
                ...
                jPages("../web/api/ordermanage/getOrders","itemContainer_forth","holder_forth",1,6,'','',array,function(pageId){
                    if( checked_array[pageId] != undefined ) {
                        $(".accountNumber").next().each(function () {
                            var _this = this;
                            checked_array[pageId].map(function(orderCode){
            ...
            if( type == '已完成' ){
                ...
                jPages("../web/api/ordermanage/getOrders","itemContainer_fifth","holder_fith",1,6,'','',array,function(pageId){
                    if( checked_array[pageId] != undefined ) {
                        $(".accountNumber").next().each(function () {
                            var _this = this;
                            checked_array[pageId].map(function(orderCode){
            ...
            if( type == '已取消' ){
                ...
                jPages("../web/api/ordermanage/getOrders","itemContainer_sixth","holder_sixth",1,6,'','',array,function(pageId){
                    if( checked_array[pageId] != undefined ) {
                        $(".accountNumber").next().each(function () {
                            var _this = this;
                            checked_array[pageId].map(function(orderCode){
    ...
    $(".accountTable li").click(function(){
        ...
        if ( str == '全部订单' ){
            ...
            status = 'RECIEVED';
            array[1] = status;
            jPages("../web/api/ordermanage/getOrders","itemContainer_fifth","holder_fifth",1,6,'','',array,function(pageId){
                checked_array = {};
                outport_array = {};
                checked_array.length = 0;
                if( checked_array[pageId] != undefined ) {
        ...
        if(checkstate == 'false,true'){//物流单详情网点代客下单
            if( isTop ){
                ...
                if( data.payTime != undefined ){
                    //邮豆支付时间
                    $( $(".detailOne .checkDetailOne dl:nth-child(1)").children()[10] ).find("span").html( handleDate_prev( new Date(data.payTime) ) + " " + handleDate_fuck_jiangtao( new Date(data.payTime) ));
                    //支付完成时间
                    $( $(".detailOne .checkDetailOne dl:nth-child(1)").children()[11] ).find("span").html(handleDate_prev( new Date(data.payTime) ) + " " + handleDate_fuck_jiangtao( new Date(data.payTime) ));
                }
                //快递信息
                //-------||||||||||------------
                var post_html = '';
                ...
                if( data.payTime != undefined ){
                    //邮豆支付时间
                    $( $(".detailFour .checkDetailFour dl:nth-child(1)").children()[10] ).find("span").html( handleDate_prev( new Date(data.payTime) ) + " " + handleDate_fuck_jiangtao( new Date(data.payTime) ));
                    //支付完成时间
            ...
            if( isTop ){
                ...
                if( data.payTime != undefined ){
                    //邮豆支付时间
                    $( $(".detailTwo .checkDetailTwo dl:nth-child(1)").children()[10] ).find("span").html( handleDate_prev( new Date(data.payTime) ) + " " + handleDate_fuck_jiangtao( new Date(data.payTime) ));
                    //支付完成时间
                    $( $(".detailTwo .checkDetailTwo dl:nth-child(1)").children()[11] ).find("span").html(handleDate_prev( new Date(data.payTime) ) + " " + handleDate_fuck_jiangtao( new Date(data.payTime) ));
                }
                //快递信息
                var post_html = '';
                var post_count = 0;
                ...
                if( data.payTime != undefined ){
                    ...
                    $( $(".detailFour .checkDetailFour dl:nth-child(1)").children()[10] ).find("span").html( handleDate_prev( new Date(data.payTime) ) + " " + handleDate_fuck_jiangtao( new Date(data.payTime) ));
                    //支付完成时间
                    $( $(".detailFour .checkDetailFour dl:nth-child(1)").children()[11] ).find("span").html(handleDate_prev( new Date(data.payTime) ) + " " + handleDate_fuck_jiangtao( new Date(data.payTime) ));
                }
                //快递信息
                var post_html = '';
                var post_count = 0;
            ...
            if( isTop ){
                ...
                $(".enterpriseAddress").html(data.pickInfo.enterpriseName);
                ...
                $(".orderStatusDetail").html(orderStatus);
                //原始金额
                $(".oldPrice").html((data.orderOldPrice).toFixed(2));
                //优惠金额
                $(".accountPrice").html(handleUndefined(data.orderPrePrice));
                //交易金额
                $(".orderPrice").html((data.orderPrice).toFixed(2));
                //邮豆支付金额
                $(".ucoinPrice").html((data.orderPrice).toFixed(2));
                //金钱支付金额
                $(".moneyPayTime").html(0);
                //邮豆支付时间
                if(data.payTime !=undefined){
    ...
    $(document).on("click",".custReturn",function(){
        ...
        $("#returnMoneyNum").html(data.orderPrice + '邮豆');//订单金额
        //商品列表
        var goodsListhtml = '';
        data.orderGoodsList.map(function(good){
            goodsListhtml += '<dd>';
            ...
            goodsListhtml += '<abbr><span class="eachPrice">' + ( good.goodsInfoOldPrice).toFixed(2) + '</span>邮豆</abbr>';
            goodsListhtml += '<abbr><span class="sumPrice">' + ( good.goodsInfoSumPrice).toFixed(2) + '</span>邮豆</abbr>';
            goodsListhtml += '<abbr><input class="itemNumReduce" type="button" value="-"><input maxlength="2" readonly="readonly" class="itemNum" value="1" type="text"><input class="itemNumAdd" type="button" value="+" /></abbr>';
            goodsListhtml += '</dd>';
        ...
        $(".eachPrice").each(function(){
            returnPrice = parseInt(returnPrice) + parseInt($(this).html());
        });
        $("#returnPrice").html(returnPrice);
    });
    ...
    $("#accExport").click(function(){
        window.location.href = '../web/api/exportExcel/orderDown?orderNoString=all' + '&status=' + status + "&username=" + username + "&contactPhone=" + contactPhone + "&createStart=" + createStart + "&createEnd=" + createEnd + "&payStart=" + payStart + "&payEnd=" + payEnd + "&enterpriseId=" + enterpriseId;
...
function appendHtml(str,type,data,perpage){
    var html = '';
    console.log(perpage,str,type,data,perpage);
    for(var i=0;i<perpage;i++){
        if( type == 1 ){
            ...
            try{
                html += '<td style="display:inline-block; width:1550px; height:60px; text-align:left; height:60px;background: #edf3f8; line-height:60px; padding-left:50px; position:relative;"><input name="accountNumberSelect" class="accountNumber" type="checkbox" />订单号：<span style="margin-right: 50px;" id="accNum">' + data[i].orderCode + '</span>所属网点：<span>' + handleUndefined(data[i].pickInfo.enterpriseName) + '</span>';
            }catch(e){
                html += '<td style="display:inline-block; width:1550px; height:60px; text-align:left; height:60px; background: #edf3f8; line-height:60px; padding-left:50px; position:relative;"><input name="accountNumberSelect" class="accountNumber" type="checkbox" />订单号：<span style="margin-right: 50px;" id="accNum">' + data[i].orderCode + '</span>所属网点：<span>暂无网点</span>';
            }
            if( isTop ){
                if( orderStatus == '待付款' ){
            ...
            html += '</td>';
            html += '<td style="display:inline-block; width:125px; height:100px;"><section style="margin-top:20px;margin-left: 0px!important;">' + data[i].orderOldPrice.toFixed() + '邮豆<br />(&nbsp;<span>'
            var goodsInfoNum = 0;
            data[i].orderGoodsList.map(function(object){
            ...
            if( data[i].selfPick == true ){
                html += '<td style="display:inline-block; width:260px; height:100px;"><section style="margin-top:10px;">用户名：' + handleUndefined(data[i].customerInfo.username) + '<span></span><br />收件人：<span class="pickName">' + handleUndefined(data[i].customerInfo.fullName) + '</span><br />';
                html += '联系电话：<span class="pickPhone">' + handleUndefined(data[i].customerInfo.contactPhone) + '</span><br /></section></td>';
            }
            else if( data[i].selfPick == false ){
                html += '<td style="display:inline-block; width:260px; height:100px;"><section style="margin-top:10px;">用户名：' + handleUndefined(data[i].customerInfo.username) + '<span></span><br />收件人：<span class="pickName">' + handleUndefined(data[i].shippingPerson) + '</span><br />';
                html += '联系电话：<span class="pickPhone">' + handleUndefined(data[i].shippingMobile) + '</span><br /></section></td>';
            }
            ...
            if( orderStatus == '已完成' ){
                html += '<td style="display:inline-block; width:200px; height:100px;"><abbr style="background: #54a6de; color: #fff;display: inline-block; margin-top: 30px;padding: 2px;border-radius: 2px;" class="orderStatus" data-orderStatus="' + data[i].orderStatus + '">' + orderStatus + '</abbr></td>';
            }else{
            ...
            try {
                html += '<td style="display:inline-block; width:120px; text-align: center; height:100px; line-height:80px;overflow: hidden"><span class="checkstate" data-storeId="' + data[i].business.businessId + '">' + handleUndefined(data[i].business.businessName) + '</span></td></tr>';
            }
            catch(e){
                html += '<td style="display:inline-block; width:120px; text-align: center; height:100px; line-height:80px;overflow: hidden"><span class="checkstate">' + handleUndefined(data[i].pickInfo.enterpriseName) + '</span></td></tr>';
            }
        }
